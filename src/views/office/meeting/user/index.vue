...
<script>
import { listUser, getUser, delUser, addUser, updateUser } from "@/api/office/meeting/user";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getUser(meetingId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户会议编号为"' + meetingIds + '"的数据项？').then(function() {
        return delUser(meetingIds);
      }).then(() => {
        this.getList();
